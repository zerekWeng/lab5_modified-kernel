...
void tcp_delack_timer_handler(struct sock *sk)
{
	...
	if (inet_csk_ack_scheduled(sk)) {
		if (!inet_csk_in_pingpong_mode(sk)) {
			...
			// LAB5 change: don't exponential back off
		} else {
			...
			// LAB5 change: don't go back to minimum ack time out.
		}
...
out_reset_timer:
	...
	if (sk->sk_state == TCP_ESTABLISHED &&
		...
		// 		       TCP_RTO_MAX);
		// LAB5 change
		icsk->icsk_rto = clamp(__tcp_set_rto(tp), tcp_rto_min(sk), ~0);
